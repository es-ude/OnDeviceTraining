...
One backward fixture (Task 3):

  LN-C "native backward": reuses LN-A's input/gamma geometry and adds a
    BFP-stored dy (m = 8, e = 8, grouped {4, 2}) whose four stored exponents
    are all DIFFERENT and whose values are non-uniform INSIDE every BFP block
    (the vacuity rule from generate_expected_layernorm_sym_bwd.py: uniform dy
    degenerates dx to rounding noise). Emitted gold: dgamma/dbeta as the raw
    FLOAT32 increments (the ACC epilogue's add into a zeroed FLOAT32 grad is
    exact, so the C test asserts them byte-identical) and dx as the packed
    propLoss wire at the {4, 2} m = 6 propLossQ geometry.
...
# ---- LN-C: backward fixture (native, all-BFP) ----
#
# x/gamma are LN-A's (backward recomputes the stats from the SAME forward
# input, R-N4). dy gets its own grid: grouped {4, 2} with four DIFFERENT
# stored exponents (bias 127 -> block scales 2^-1, 2, 1, 2^-2), so dbeta's
# j-outer/g-inner segment walk folds across a block boundary on EVERY j.
# Codes are pairwise distinct INSIDE every block (per-block non-uniformity,
# asserted below): with per-block-uniform dy, dx degenerates to rounding
# noise and scatter/reduction mutations become invisible (the vacuity rule
# from generate_expected_layernorm_sym_bwd.py). All dequants are small dyadic
# rationals -- 20, -36 / 30, 176 / -55, 30 / -22.5, 5.25 -- exact in float32.
LN_C_DY_CODES = [40, -72, 15, 88, -55, 30, -90, 21]
LN_C_DY_EXPS = [126, 128, 127, 125]
LN_C_DY_QC = {"mantissa_bits": 8, "exponent_bits": 8, "group_size": 2}
...
# ---- Task 3: backward emulation (layerNormCalcGammaGradsBfp /
# layerNormCalcBetaGradsBfp / layerNormCalcPropLossBfp in LayerNorm.c),
# statement for statement in the C loop/op order -- float addition order
# matters, so g-outer/j-inner (dgamma) vs j-outer/g-inner (dbeta) is part of
# the contract, not a style choice. ----


def stats_bfp(x_codes, x_exps, x_qc, g_count, n):
    """layerNormAllGroupStats' BFP arm: Reduce-emulated mean/var, then
    invSigma = rsqrtFloat32(var, eps) = 1 / sqrt(var + eps)."""
    means = block_mean_bfp(x_codes, x_exps, x_qc, g_count, n)
    variances = block_var_bfp(x_codes, x_exps, x_qc, means, g_count, n)
    inv_sigma = [f32_div(f32(1.0), sqrt_f32(f32_add(v, EPS))) for v in variances]
    return means, inv_sigma


def dgamma_bfp(x_codes, x_exps, x_qc, dy_codes, dy_exps, dy_qc, g_count, n):
    """layerNormCalcGammaGradsBfp: raw memset to zero, then g-outer/j-inner
    out[j] = f32_add(out[j], f32_mul(dy, nval)) -- per-element float32, no
    int partials (a mixed mantissa x float product has no same-exponent int32
    partial)."""
    ...
    for g in range(g_count):
        for j in range(n):
            off = phys_offset(g, j, n)
            x = deq(x_codes[off], x_exps, x_qc, off)
            nval = f32_mul(f32_sub(x, means[g]), inv_sigma[g])
            dy = deq(dy_codes[off], dy_exps, dy_qc, off)
            out[j] = f32_add(out[j], f32_mul(dy, nval))
    return out


def dbeta_bfp(dy_codes, dy_exps, dy_qc, g_count, n):
    """layerNormCalcBetaGradsBfp: pure mantissa VALUE-sum over groups, walked
    j-outer/g-inner with ONE int32 partial per same-exponent segment folded
    through ldexpf (R-P4). The strided walk crosses a dy block boundary on
    every j here, so the fold-on-group-change clause fires N times."""
    bias = bfp_bias(dy_qc)
    limit = 2 ** 31 - 1
    out = []
    for j in range(n):
        ...
        for g in range(g_count):
            off = phys_offset(g, j, n)
            grp = _bfp_group_of(off, dy_qc["group_size"])
            if g == 0:
                current_group = grp
            elif grp != current_group:
                acc = f32_add(acc, ldexp_f32(float(partial), dy_exps[current_group] - bias))
                partial = 0
                current_group = grp
            partial += dy_codes[off]
            assert abs(partial) <= limit, (
                f"dbeta_bfp: segment partial {partial} leaves int32 -- the C kernel "
                "guarantees this bound via bfpValidateSumHeadroom, never via int64")
        acc = f32_add(acc, ldexp_f32(float(partial), dy_exps[current_group] - bias))
        out.append(acc)
    return out
...
def dx_bfp(x_codes, x_exps, x_qc, dy_codes, dy_exps, dy_qc, gamma_codes, gamma_exps, gamma_qc,
           g_count, n):
    ...
    for g in range(g_count):
        ...
        for j in range(n):
            ...
            gv = deq(gamma_codes[j], gamma_exps, gamma_qc, j)
            dn = f32_mul(dy, gv)
            sum_dn = f32_add(sum_dn, dn)
            sum_dn_n = f32_add(sum_dn_n, f32_mul(dn, nval))
        mean_dn = f32_div(sum_dn, f32(n))
        mean_dn_n = f32_div(sum_dn_n, f32(n))
        for j in range(n):
            off = phys_offset(g, j, n)
            x = deq(x_codes[off], x_exps, x_qc, off)
            nval = f32_mul(f32_sub(x, means[g]), inv_sigma[g])
            ...
            gv = deq(gamma_codes[j], gamma_exps, gamma_qc, j)
            dn = f32_mul(dy, gv)
            dx[off] = f32_mul(inv_sigma[g],
                              f32_sub(f32_sub(dn, mean_dn), f32_mul(nval, mean_dn_n)))
    return dx


def reference_layer_norm_backward(x_deq, gamma_deq, beta_deq, dy_deq):
    """float64 torch autograd on the exact dequants -- the INDEPENDENT oracle
    for all three backward emulations (shares no code with them)."""
    x = torch.tensor(x_deq, dtype=torch.float64, requires_grad=True)
    w = torch.tensor(gamma_deq, dtype=torch.float64, requires_grad=True)
    b = torch.tensor(beta_deq, dtype=torch.float64, requires_grad=True)
    y = torch.nn.functional.layer_norm(x.reshape(ROWS, COLS), (COLS,), weight=w, bias=b,
                                       eps=float(EPS))
    y.backward(torch.tensor(dy_deq, dtype=torch.float64).reshape(ROWS, COLS))
    return x.grad.flatten().tolist(), w.grad.tolist(), b.grad.tolist()


def check_per_block_non_uniform(name, codes, group_size):
    """The vacuity rule, per BFP BLOCK (stronger than per norm group -- every
    norm group here contains two blocks): a uniform block shares one scale, so
    uniform codes mean uniform values and dx collapses toward rounding noise."""
    for g0 in range(0, len(codes), group_size):
        block = codes[g0:g0 + group_size]
        assert len(set(block)) > 1, (
            f"{name}: block starting at {g0} is uniform ({block}) -- degenerate/vacuous "
            "backward fixture (uniform dy degenerates dx to rounding noise)")


def backward_parts():
    """Header body for the LN-C backward fixture."""
    check_per_block_non_uniform("LN-C dy codes", LN_C_DY_CODES, LN_C_DY_QC["group_size"])
    check_non_uniform("LN-C dy exponents", LN_C_DY_EXPS)

    c_dgamma = dgamma_bfp(LN_A_X_CODES, LN_A_X_EXPS, LN_A_X_QC, LN_C_DY_CODES, LN_C_DY_EXPS,
                          LN_C_DY_QC, ROWS, COLS)
    c_dbeta = dbeta_bfp(LN_C_DY_CODES, LN_C_DY_EXPS, LN_C_DY_QC, ROWS, COLS)
    c_dx_raw = dx_bfp(LN_A_X_CODES, LN_A_X_EXPS, LN_A_X_QC, LN_C_DY_CODES, LN_C_DY_EXPS,
                      LN_C_DY_QC, LN_A_GAMMA_CODES, LN_A_GAMMA_EXPS, LN_A_GAMMA_QC, ROWS, COLS)
    c_dx_codes, c_dx_exps = pack_output(c_dx_raw)

    # Independent oracle: float64 autograd on the exact dequants. dgamma/dbeta
    # are pure float32 sums of exact values -> 1e-3 absolute; dx additionally
    # crosses the m = 6 pack -> half the coarsest pack step + 1e-3.
    x_deq = [float(deq(c, LN_A_X_EXPS, LN_A_X_QC, i)) for i, c in enumerate(LN_A_X_CODES)]
    gamma_deq = [float(deq(c, LN_A_GAMMA_EXPS, LN_A_GAMMA_QC, i))
                 for i, c in enumerate(LN_A_GAMMA_CODES)]
    beta_deq = [float(deq(c, LN_A_BETA_EXPS, LN_A_BETA_QC, i))
                for i, c in enumerate(LN_A_BETA_CODES)]
    dy_deq = [float(deq(c, LN_C_DY_EXPS, LN_C_DY_QC, i)) for i, c in enumerate(LN_C_DY_CODES)]
    ref_dx, ref_dgamma, ref_dbeta = reference_layer_norm_backward(x_deq, gamma_deq, beta_deq,
                                                                  dy_deq)
    for i, (got, want) in enumerate(zip(c_dgamma, ref_dgamma)):
        assert abs(float(got) - want) <= 1e-3, (
            f"LN-C dgamma[{i}]: emulation says {float(got)}, torch autograd says {want} -- "
            "the emulation and the definition disagree")
    for i, (got, want) in enumerate(zip(c_dbeta, ref_dbeta)):
        assert abs(float(got) - want) <= 1e-3, (
            f"LN-C dbeta[{i}]: emulation says {float(got)}, torch autograd says {want} -- "
            "the emulation and the definition disagree")
    dx_qc = {"mantissa_bits": OUT_MANTISSA_BITS, "exponent_bits": OUT_EXPONENT_BITS,
             "group_size": OUT_GROUP_SIZE}
    c_dx_deq = [float(deq(c, c_dx_exps, dx_qc, i)) for i, c in enumerate(c_dx_codes)]
    dx_bound = 0.5 * max_group_scale(c_dx_exps, OUT_EXPONENT_BITS) + 1e-3
    for i, (got, want) in enumerate(zip(c_dx_deq, ref_dx)):
        assert abs(got - want) <= dx_bound, (
            f"LN-C dx[{i}]: packed dx dequantizes to {got}, torch autograd says {want} "
            f"(bound {dx_bound}) -- the emulation and the definition disagree")

    return [
        "\n/* LN-C: native backward on LN-A's x/gamma -- BFP dy (grouped {4, 2}, four\n"
        " * distinct exponents, per-block non-uniform values); dgamma/dbeta are the\n"
        " * raw FLOAT32 increments (exact ACC into a zeroed FLOAT32 grad), dx is the\n"
        " * packed propLoss wire at the produced-wire geometry (m = 6, {4, 2}). */\n",
        emit_int32_array("kLnBfpCDyCodes", torch.tensor(LN_C_DY_CODES)),
        emit_uint8_array("kLnBfpCDyExponents", LN_C_DY_EXPS),
        emit_float_array("kLnBfpCDgamma", torch.tensor([float(v) for v in c_dgamma],
                                                       dtype=torch.float32)),
        emit_float_array("kLnBfpCDbeta", torch.tensor([float(v) for v in c_dbeta],
                                                      dtype=torch.float32)),
        emit_int32_array("kLnBfpCDxCodes", torch.tensor(c_dx_codes)),
        emit_uint8_array("kLnBfpCDxExponents", c_dx_exps),
    ]
...
def main() -> None:
    ...
    # Include-guard opener/closer live HERE so they wrap forward AND backward
    # parts (Task 2 review: inside forward_parts() they would strand every
    # backward array after the #endif).
    parts = [
